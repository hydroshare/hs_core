...
class TestCoreMetadata(TestCase):
    ...
    def test_auto_element_creation(self):
        ...
        self.assertEqual(self.res.metadata.creators.all().first().name, self.res.creator.get_full_name(), msg='resource creator did not match')
    ...
    def test_creator(self):
        ...
        creator = Creator.objects.filter(name='John Smith', object_id=self.res.metadata.id, content_type__pk=metadata_type.id).first()
        kwargs = {'email':'jsmith@gmail.com', 'order': 2}
        resource.update_metadata_element(self.res.short_id,'creator', creator.id, **kwargs)

        self.assertIn('jsmith@gmail.com', [cr.email for cr in self.res.metadata.creators.all()], msg="Creator 'John Smith' email was not found")
        ...
        self.assertRaises(Exception, lambda :resource.delete_metadata_element(self.res.short_id, 'creator', self.user.id ))
    ...
    def test_contributor(self):
        ...
        self.assertIn('John Smith', [con.name for con in self.res.metadata.contributors.all()], msg="Contributor 'John Smith' was not found")

        # update contributor John Smith
        metadata_type = ContentType.objects.get_for_model(self.res.metadata)
        con_john = Contributor.objects.filter(name='John Smith', object_id=self.res.metadata.id, content_type__pk=metadata_type.id).first()
        kwargs = {'email':'jsmith@gmail.com'}
        resource.update_metadata_element(self.res.short_id,'contributor', con_john.id, **kwargs)

        self.assertIn('jsmith@gmail.com', [con.email for con in self.res.metadata.contributors.all()], msg="Contributor 'John Smith' email was not found")
    ...
    def test_party_external_links(self):
        ...
        kwargs = {'name':'Lisa Howard', 'email':'lasah@yahoo.com', 'profile_links': [{'type':'yahooProfile', 'url': 'http://yahoo.com/LH001'}]}
        ...
        self.assertRaises(Exception, lambda : resource.update_metadata_element(self.res.short_id,'creator', cr_lisa.id, **kwargs))
        ...
        self.assertIn('yahooProfile', [link.type for link in cr_lisa.external_links.all()], msg="Creator 'Lisa' does not have link type: yahooProfile")
        self.assertIn('usuProfile', [link.type for link in cr_lisa.external_links.all()], msg="Creator 'Lisa' does not have link type: usuProfile")
        self.assertIn('http://yahoo.com/LH001', [link.url for link in cr_lisa.external_links.all()], msg="Creator 'Lisa' does not have link url: 'http://yahoo.com/LH001'")
        self.assertIn('http://usu.edu/profile/LH001', [link.url for link in cr_lisa.external_links.all()], msg="Creator 'Lisa' does not have link url: 'http://usu.edu/profile/LH001'")
        ...
        # lisa should have one external profile link
        self.assertEqual(con_lisa.external_links.all().count(), 1, msg="contributor Lisa does not have 1 external link.")

        # test that trying to add another link of the same type should raise exception
        kwargs = {'profile_links': [{'type':'yahooProfile', 'url': 'http://yahoo.com/LH002'}]}
        self.assertRaises(Exception, lambda : resource.update_metadata_element(self.res.short_id,'contributor', con_lisa.id, **kwargs))
        ...
        # lisa should have 2 external profile links at this point
        self.assertEqual(con_lisa.external_links.all().count(), 2, msg="Contributor Lisa does not have 2 external link.")

        self.assertIn('yahooProfile', [link.type for link in con_lisa.external_links.all()], msg="Contributor 'Lisa' does not have link type: yahooProfile")
        self.assertIn('usuProfile', [link.type for link in con_lisa.external_links.all()], msg="Contributor 'Lisa' does not have link type: usuProfile")
    ...
    def test_title(self):
        ...
        self.assertRaises(Exception, lambda : resource.delete_metadata_element(self.res.short_id,'title', self.res.metadata.title.id))
    ...
    def test_date(self):
        ...
        self.assertRaises(Exception, lambda : resource.create_metadata_element(self.res.short_id,'date', type='created', start_date="8/10/2014"))

        # add another date of type 'modified' - which should raise an exception
        self.assertRaises(Exception, lambda : resource.create_metadata_element(self.res.short_id,'date', type='modified', start_date="8/10/2014"))

        # add another date of type 'published' - which should raise an exception since the resource is not yet published.
        self.assertRaises(Exception, lambda : resource.create_metadata_element(self.res.short_id,'date', type='published', start_date="8/10/2014"))

        # make the resource published and then add the date type published - which should work
        self.res.published_and_frozen = True
        self.res.save()
        ...
        self.res.public = False
        self.res.save()
        self.assertRaises(Exception, lambda : resource.create_metadata_element(self.res.short_id,'date', type='available', start_date="8/10/2014"))

        # make the resource public and then add available date
        self.res.public = True
        self.res.save()
        resource.create_metadata_element(self.res.short_id,'date', type='available', start_date="8/10/2014")
        self.assertIn('available', [dt.type for dt in self.res.metadata.dates.all()], msg="Date element type 'Available' does not exist")
        ...
        self.assertIn('valid', [dt.type for dt in self.res.metadata.dates.all()], msg="Date element type 'Valid' does not exist")

        # trying to add an existing date type element should raise an exception
        self.assertRaises(Exception, lambda :resource.create_metadata_element(self.res.short_id,'date', type='valid', start_date="9/10/2014"))

        # add another date of type 'none-type' that is not one of the date types allowed - which should raise an exception.
        self.assertRaises(Exception, lambda : resource.create_metadata_element(self.res.short_id,'date', type='none-type', start_date="8/10/2014"))
        ...
        self.assertRaises(Exception, lambda :resource.update_metadata_element(self.res.short_id,'date', dt_created.id, start_date='8/10/2014'))
        ...
        resource.update_metadata_element(self.res.short_id,'date', dt_valid.id, start_date='8/10/2011', end_date='8/11/2012')
        dt_valid = self.res.metadata.dates.all().filter(type='valid').first()
        ...
        dt_modified = self.res.metadata.dates.all().filter(type='modified').first()
        self.assertRaises(Exception, lambda : resource.delete_metadata_element(self.res.short_id,'date', dt_modified.id))
    ...
    def test_description(self):
        ...
        # update the abstract
        resource.update_metadata_element(self.res.short_id,'description', self.res.metadata.description.id, abstract='Updated generic resource')
        self.assertEqual(self.res.metadata.description.abstract, 'Updated generic resource')

        # test adding a 2nd descripotion element - should raise an exception
        self.assertRaises(Exception, lambda :resource.create_metadata_element(self.res.short_id,'description', abstract='new abstract for the resource'))

        # delete the abstract - should raise exception
        self.assertRaises(Exception, lambda :resource.delete_metadata_element(self.res.short_id,'description', self.res.metadata.description.id))
    ...
    def test_format(self):
        ...
        self.assertIn(format_csv, [fmt.value for fmt in self.res.metadata.formats.all()], msg="Format element with value of %s does not exist." % format_csv)
        ...
        self.assertIn(format_csv, [fmt.value for fmt in self.res.metadata.formats.all()], msg="Format element with value of %s does not exist." % format_csv)
        self.assertIn(format_zip, [fmt.value for fmt in self.res.metadata.formats.all()], msg="Format element with value of %s does not exist." % format_zip)
    ...
    def test_identifier(self):
        ...
        resource.create_metadata_element(self.res.short_id,'identifier', name='someIdentifier', url="http://some.org/001")
        self.assertEqual(self.res.metadata.identifiers.all().count(), 2, msg="Number of identifier elements not equal to 2.")
        ...
        self.assertRaises(Exception, lambda :resource.update_metadata_element(self.res.short_id, 'identifier', hs_idf.id,
                                                                     name='anotherIdentifier'))
        self.assertRaises(Exception, lambda :resource.update_metadata_element(self.res.short_id, 'identifier', hs_idf.id,
                                                                     url='http://resources.org/001'))

        # test adding a identifier with name 'DOI' when the resource does not have a DoI - should raise an exception
        self.res.doi = None
        self.res.save()
        self.assertRaises(Exception, lambda :resource.create_metadata_element(self.res.short_id,'identifier',
        ...
        self.assertRaises(Exception, lambda :resource.update_metadata_element(self.res.short_id, 'identifier', doi_idf.id,
                                                                     name='DOI-1'))
        ...
        hs_idf = self.res.metadata.identifiers.all().filter(name='hydroShareIdentifier').first()
        self.assertRaises(Exception, lambda :resource.delete_metadata_element(self.res.short_id, 'identifier', hs_idf.id))

        # test that the DOI identifier can't be deleted when the resource has a DOI - should cause exception
        doi_idf = self.res.metadata.identifiers.all().filter(name='DOI').first()
        self.assertRaises(Exception, lambda :resource.delete_metadata_element(self.res.short_id, 'identifier', doi_idf.id))
    ...
    def test_publisher(self):
        if not self.res.metadata.publisher:
            ...
            # delete the file the resource has and then try to change the name of the publisher to anything you want - that should work
    ...
    def test_rights(self):
        ...
        resource.create_metadata_element(self.res.short_id,'rights', statement='This is the rights statement for this resource', url='http://rights.ord/001')
        ...
        self.assertRaises(Exception, lambda : resource.delete_metadata_element(self.res.short_id,'rights', self.res.metadata.rights.id))

        # should be able to update the rights element
        resource.update_metadata_element(self.res.short_id,'rights', self.res.metadata.rights.id, statement='This is the modified rights statement for this resource', url='http://rights-modified.ord/001')
        self.assertEqual(self.res.metadata.rights.statement, 'This is the modified rights statement for this resource', msg="Statement of rights did not match.")
        self.assertEqual(self.res.metadata.rights.url, 'http://rights-modified.ord/001', msg="URL of rights did not match.")
    ...
    def test_source(self):
        ...
        self.assertIn('http://hydroshare.org/resource/0001', [src.derived_from for src in self.res.metadata.sources.all()], msg="Source element with derived from avlue of %s does not exist." % 'http://hydroshare.org/resource/0001')

        #test update
        src_1 = self.res.metadata.sources.all().filter(derived_from='http://hydroshare.org/resource/0001').first()
        resource.update_metadata_element(self.res.short_id, 'source', src_1.id, derived_from='http://hydroshare.org/resource/0002')
        self.assertIn('http://hydroshare.org/resource/0002', [src.derived_from for src in self.res.metadata.sources.all()], msg="Source element with derived from avlue of %s does not exist." % 'http://hydroshare.org/resource/0002')

        # add another source element of string type
        resource.create_metadata_element(self.res.short_id,'source', derived_from='This resource has been derived from another resource')
        ...
        self.assertRaises(Exception, lambda :resource.create_metadata_element(self.res.short_id,'source', derived_from='http://hydroshare.org/resource/0002'))
    ...
    def test_subject(self):
        ...
        self.assertIn('sub-1', [sub.value for sub in self.res.metadata.subjects.all()], msg="Subject element with value of %s does not exist." % 'sub-1')
        ...
        self.assertIn('sub-1', [sub.value for sub in self.res.metadata.subjects.all()], msg="Subject element with value of %s does not exist." % 'sub-1')
        self.assertIn('sub-2', [sub.value for sub in self.res.metadata.subjects.all()], msg="Subject element with value of %s does not exist." % 'sub-1')

        # add another subject element with duplicate value- should raise exception
        self.assertRaises(Exception, lambda :resource.create_metadata_element(self.res.short_id,'subject', value='sub-2'))
        ...
        self.assertIn('sub-1.1', [sub.value for sub in self.res.metadata.subjects.all()], msg="Subject element with value of %s does not exist." % 'sub-1.1')
    ...
    def test_type(self):
        ...
        if not self.res.metadata.type:
            ...
            self.assertEqual(self.res.metadata.type.url, "http://hydroshare.org/generic", msg="Resource type url did not match.")

        # adding a 2nd type element should raise exception
        self.assertRaises(Exception, lambda :resource.create_metadata_element(self.res.short_id, 'type', url="http://hydroshare.org/generic"))

        # it should be possible to update the type element
        resource.update_metadata_element(self.res.short_id, 'type', self.res.metadata.type.id, url="http://hydroshare2.org/generic")
        self.assertEqual(self.res.metadata.type.url, "http://hydroshare2.org/generic", msg="Resource type url did not match.")

        # deleting of type element is not allowed.
        self.assertRaises(Exception, lambda :resource.delete_metadata_element(self.res.short_id, 'type', self.res.metadata.type.id))
    ...
    def test_get_xml(self):
        ...
        self.assertEqual(container.get('{%s}about' % self.res.metadata.NAMESPACES['rdf']), res_uri, msg="'Description' element attribute value did not match.")
